import Image from 'next/image';
import ButtonComponent from './components/buttoncomponent';

export default function Home() {
  return (
    <main className="h-full overflow-hidden bg-white">
      {/* Desktop Layout */}
      <div className="hidden lg:flex h-full">
        {/* Left Side - Content */}
        <div className="flex-1 flex items-center justify-center bg-white">
          <div className="max-w-lg text-center space-y-6 px-8">
            {/* Logo */}
            <div className="flex justify-center">
              <Image
                src="/holychildlogo.png"
                alt="Holy Child College logo"
                width={150}
                height={150}
                priority
              />
            </div>

            {/* Headline */}
            <h1 className="text-[#7A3B33] font-serif text-4xl leading-tight">
              <span className="block">Create Lasting</span>
              <span className="block">Memories</span>
              <span className="block text-2xl mt-2">with</span>
              <span className="block text-2xl">Custom Frames</span>
            </h1>

            {/* Description */}
            <p className="text-base text-gray-700 leading-relaxed">
              Design unique frames for Holy Child College of Education&apos;s 80th Anniversary. Preserve your cherished moments in style and honor a legacy.
            </p>

            {/* Button */}
            <div className="pt-2">
              <ButtonComponent href="/uploadpic">Start Framing</ButtonComponent>
            </div>
          </div>
        </div>

        {/* Right Side - Frame Preview */}
        <div className="flex-1 bg-[#7A3B33] flex items-center justify-center">
          <div className="max-w-md">
            <Image
              src="/poster.svg"
              alt="Sample framed poster"
              width={350}
              height={450}
              className="object-contain"
            />
          </div>
        </div>
      </div>

      {/* Mobile Layout */}
      <div className="lg:hidden h-full flex items-center justify-center overflow-hidden">
        <div className="w-full max-w-sm flex flex-col items-center text-center space-y-4 py-6 px-6">
          {/* Logo */}
          <div>
            <Image
              src="/holychildlogo.png"
              alt="Holy Child College logo"
              width={100}
              height={100}
              priority
            />
          </div>

          {/* Headline */}
          <h1 className="text-[#7A3B33] font-serif text-2xl leading-tight">
            <span className="block">Create Lasting Memories</span>
            <span className="block text-xl mt-1">with Custom Frames</span>
          </h1>

          {/* Description */}
          <p className="text-sm text-gray-700 px-2">
            Design unique frames for Holy Child College of Education&apos;s 80th Anniversary.
          </p>

          {/* Poster / framed image placeholder */}
          <div className="w-48 bg-[#6a3f36] p-3 rounded-sm shadow-lg">
            <div className="bg-white rounded-sm p-2 flex items-center justify-center">
              <Image
                src="/poster.svg"
                alt="Sample framed poster"
                width={160}
                height={160}
                className="object-cover"
              />
            </div>
          </div>
          <div className="w-full flex justify-center">
            <ButtonComponent href="/uploadpic">Start Framing</ButtonComponent>
          </div>
        </div>
      </div>
    </main>
  );
}
